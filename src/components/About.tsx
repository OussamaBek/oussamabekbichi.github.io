  ...
  return (
    ...
    >
      <motion.div
        ...
        className="mt-8 grid gap-4 sm:mt-10 sm:gap-6 lg:grid-cols-[1.1fr_0.9fr]"
      >
        <motion.div
          variants={fadeUp}
          className="rounded-lg border border-slate-200/80 bg-white/[0.72] p-5 shadow-soft backdrop-blur-xl transition-colors dark:border-white/10 dark:bg-white/[0.055] dark:shadow-soft-dark sm:p-8"
        >
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            {qualities.map((quality) => (
              <span
                key={quality}
                className="max-w-full break-words rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-2.5 py-1.5 text-xs font-medium text-emerald-800 dark:border-emerald-300/20 dark:bg-emerald-300/[0.08] dark:text-emerald-100 sm:px-3 sm:text-sm"
              >
                {quality}
              </span>
            ))}
        ...
        <motion.div
          variants={fadeUp}
          className="rounded-lg border border-slate-200/80 bg-slate-950 p-5 text-white shadow-soft-dark dark:border-white/10 dark:bg-white/[0.06] sm:p-8"
        >
